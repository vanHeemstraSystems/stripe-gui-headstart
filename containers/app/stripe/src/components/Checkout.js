      ...
      <Container>
        ...
        >
          ...
          {cartItems.length > 0 ? <React.Fragment>
            ...
            marginBottom={6}
          >
            <Text color="darkGray" italic>{cartItems.length} distinctive item(s) for Checkout</Text>
            <Box padding={2}>
              {cartItems.map(item => (
                <Box key={item._id} padding={1}>
                  <Text color="midnight">
                    {item.name} x {item.quantity} - {process.env.REACT_APP_CURRENCY_SYMBOL}{item.quantity * item.price}                    
                  </Text>
                </Box>
              ))}
            </Box>
            ...
            <Box color="darkWash" shape="rounded" padding={4}>
              ...
              <Text align="center" italic color="darkGray">Add some items!</Text>
            </Box>
          )}
        </Box>
        {/* Confirmation Modal */}
...
const ConfirmationModal = ({orderProcessing, cartItems, closeModal, handleSubmitOrder}) => (
  ...
  >
    ...
    {orderProcessing && <Text size="sm" align="center" italic>Submitting Order ...</Text>}
  </Modal>
)
...
const Checkout = () => (
  <StripeProvider apiKey="pk_test_51K8dFIAoGYTOOZ9aAeWiSXy2o9FLEkysHiTbXxN3MJoFdB53gY8KNoDdZzvABZwAdaparZIIizuHjQMRupmwDjOO005zlIpTue">
    <Elements>
      <CheckoutForm />
    </Elements>
  </StripeProvider>
